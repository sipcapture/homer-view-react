...
export default function formatMessagesResponse(rawData) {
  const data = _.cloneDeep(maybe(rawData.data, data => data.messages) || []);
  ...
  const dataTable = _.map(data, (val, index) => [
    ...
    {
      key: "srcIp",
      label: checkAlias(val, rawData.data.alias).srcAlias
    },
    { key: "srcPort", label: val.srcPort },
    {
      key: "dstIp",
      label: checkAlias(val, rawData.data.alias).dstAlias
    },
...
function checkAlias(address, aliasObj) {
  ...
  const srcAlias = _.find(
    aliasObject,
    (val, index) =>
      index === `${address.srcIp}:${address.srcPort}` || index === address.srcIp
  );
  const dstAlias = _.find(
    aliasObject,
    (val, index) =>
      index === `${address.dstIp}:${address.dstPort}` || index === address.dstIp
  );
